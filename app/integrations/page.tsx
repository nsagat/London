import { Plug, Check, Plus, Sparkles } from "lucide-react";
    ...
    <div className="mx-auto max-w-[1400px] space-y-6 px-5 py-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {INTEGRATIONS.filter((i) => !i.highlight).map((i) => (
          <Card key={i.name} className="flex items-center justify-between p-4">
            ...
            {i.connected ? (
              ...
              <button className="flex items-center gap-1 rounded-lg border border-[var(--color-border)] px-2.5 py-1.5 text-xs font-medium text-[var(--color-ink-soft)] hover:bg-[var(--color-surface-muted)]">
        ...
        <Card className="flex items-center justify-center border-dashed p-4">
          <button className="flex items-center gap-1.5 text-sm font-medium text-[var(--color-muted)]">
